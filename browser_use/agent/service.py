...
class Agent:
    ...
    async def get_next_action(self, input_messages: list[BaseMessage],
        ...
        self._log_response(parsed, input_messages, step_info)
        self.n_steps += 1

        return parsed

    def _log_response(self, response: AgentOutput, input_messages: list[BaseMessage],
        ...
        if self.agent_id:
            folder_path = f"trajectory/case-{self.agent_id}"
            ...
            with open(input_file_path, "w") as input_file:
                json.dump(messages_to_dict(input_messages), input_file, ensure_ascii=False, indent=4)
            output_file_path = f'{folder_path}/{step_info.step_number + 1}_output.json'
            with open(output_file_path, 'w', encoding='utf-8') as json_file:
                json.dump(response_json, json_file, ensure_ascii=False, indent=4)
        else:
            logger.error(f"agent is none.")
    ...
    async def run(self, max_steps: int = 100) -> AgentHistoryList:
        ...
        try:
            logger.info(f"Agent id is: {self.agent_id}, trajectory directory is: trajectory/case-{self.agent_id}.")
